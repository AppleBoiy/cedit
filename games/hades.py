...
def _load_item_names() -> dict[str, str]:
    ...
    if _ITEM_NAMES is not None:
        return _ITEM_NAMES
    catalog_path = Path(__file__).parent.parent / "data" / "hades" / "item_names.json"
...
def discover_saves() -> list[str]:
    cfg_path = Path(__file__).parent.parent / "data" / "hades.json"
...
PROFILE = GameProfile(
    key="hades",
    display_name="Hades",
    default_save_dirs=["~/Library/Application Support/Supergiant Games/Hades", "%USERPROFILE%/Saved Games/Hades"],
    file_patterns=[("Hades Saves", "Profile1.sav Profile2.sav Profile3.sav Profile4.sav Profile1_Temp.sav"), ("All Files", "*.* ")],
    quick_fields={'Darkness': ['GameState', 'Resources', 'MetaPoints'], 'Chthonic Keys': ['GameState', 'Resources', 'LockKeys'], 'Gemstones': ['GameState', 'Resources', 'Gems'], 'Titan Blood': ['GameState', 'Resources', 'SuperLockKeys'], 'Runs': ['Header', 'Runs']},
