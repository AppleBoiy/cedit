"""
cedit game profile: Hades II (Supergiant Games)

Save format: Supergiant Binary v1 (SGB1) holding save metadata, run history,
fear/shrine testaments, and game progression state.
"""
from pathlib import Path
from lib.base import GameProfile
from lib import hades_lib

_CONFIG_PATH = Path(__file__).resolve().parent.parent / "data" / "hades2.json"

class Hades2Data(dict):
    """Parsed representation of an SGB1 save, preserving original raw bytes."""
    pass

def loads(raw_bytes: bytes) -> Hades2Data:
    parsed = hades_lib.parse_sgb1_save(raw_bytes)
    wrapped = Hades2Data(parsed)
    wrapped._raw = bytes(raw_bytes)
    return wrapped

def dumps(data: Hades2Data) -> bytes:
    if not isinstance(data, Hades2Data) or not hasattr(data, "_raw"):
        raise ValueError(
            "This save was not loaded through cedit's Hades II profile (missing original file bytes)."
        )
